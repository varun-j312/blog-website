...
const homeStartingContent =
  "Hello! Welcome to my blog. This is a personal project that I made to practise working with node.js and mongodb database.";
const aboutContent =
  "Hello! My name is Varun Janardhanan. I am a budding web developer. I love programming. I like to develop browser games and make pixel art.";
...
app.get("/edit", function (req, res) {
  Post.find({}, function (err, posts) {
    res.render("home", {
      title: "Editing page",
      startingContent: "Click on edit to edit a post",
      postList: posts,
      moment: moment,
      formRoute: "/edit/search",
      linkRoute: "/edit",
      linkAction: "Edit",
    });
  });
...
app.get("/search/:searchItem", function (req, res) {
  ...
  Post.find({}, function (err, posts) {
    posts.forEach(function (post) {
      if (searchItem.includes("[")) {
        for (tag = 0; tag < queriedTags.length; tag++) {
          if (
            post.tags.toLowerCase().includes(queriedTags[tag].toLowerCase())
          ) {
            if (tag == queriedTags.length - 1) {
    ...
    res.render("home", {
      ...
      startingContent: searchItem,
      postList: queriedPosts,
      moment: moment,
      formRoute: "/search",
      linkRoute: "/posts",
      linkAction: "Read more",
